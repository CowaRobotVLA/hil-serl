...
def get_model(cfg: DictConfig, torch_dtype=None):
    ...
    import safetensors
    from serl_launcher_torch.networks.RLinf_plug.openpi.dataconfig import (
        get_openpi_config,
    )
    ...
    if config_name is None:
        raise ValueError("cfg.openpi.config_name is required.")
...
def get_sac_model(cfg: DictConfig, torch_dtype=None):
    import glob
    ...
    actor_train_config = get_openpi_config(config_name, model_path=cfg.model_path)
    actor_model_config = actor_train_config.model
    actor_model_config = OpenPi0SACConfig(**actor_model_config.__dict__)
    override_config_kwargs = cfg.openpi
    if override_config_kwargs is not None:
        for key, val in override_config_kwargs.items():
            actor_model_config.__dict__[key] = val
    ...
    model: OpenPi0ForSACActionPrediction = OpenPi0ForSACActionPrediction(actor_model_config)
    if actor_model_config.train_expert_only:
        model.freeze_vlm()
