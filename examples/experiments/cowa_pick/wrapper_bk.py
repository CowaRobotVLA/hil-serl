import copy
import time
from cowa_env.utils.rotations import euler_2_quat
from scipy.spatial.transform import Rotation as R
import numpy as np
import requests
from pynput import keyboard

from cowa_env.envs.cowa_env import cowa_env
...
class PickEnv(cowa_env):
    ...
    def go_to_reset(self, joint_reset=False):
        ...
        self._send_command(np.concatenate(95, self.currpos))
        time.sleep(0.3)
        ...
        if self.randomreset:  # randomize reset position in xy plane
            ...
            self._send_command(np.concatenate(95,reset_pose))
        else:
            reset_pose = self.resetpos.copy()
            self._send_command(np.concatenate(95,reset_pose))
        time.sleep(0.5)
